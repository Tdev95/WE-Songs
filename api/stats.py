...
def create_query(valid_args):
    ...
    query = f'SELECT hotness FROM song WHERE hotness != -1 AND artist_id = "{valid_args["artist"]}"'
...
def construct_blueprint(connector):
    ...
    def stats():
        '''
         returns a set of median, mean and standard deviation of list of hotnesses ща фкешые matching request
        '''
